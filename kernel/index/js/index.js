...
//发表留言成功的回调函数
function putMessageBack(json) {
  bodyMessage('留言成功');
  var str = messagehtml
    .replace(/@info@/g, json.rows.backinfo.msg)
    .replace(/@username@/g, json.rows.backinfo.name)
    .replace(/@time@/g, dateStr2str(json.rows.backinfo.time));
  $(str)
    .prependTo($('#lit_boxr'))
    .fadeIn(300);
...
function getMessageBack(json) {
  ...
  if (json.rows.length <= 0) {
    ...
    for (var c = 0; c < json.rows.length; c++) {
      e += messagehtml
        .replace(/@info@/g, json.rows[c].info.replace(/src="f/g, '/src="' + baseip + '/f'))
        .replace(/@username@/g, json.rows[c].username)
        .replace(/@time@/g, dateStr2str(json.rows[c].time));
    }
    $(e)
      .insertBefore(a)
      .fadeIn(300);
...
function getMovieListBack(json) {
  ...
  if (json.rows.length <= 0) {
    ...
    for (var i = 0; i < json.rows.length; i++) {
      str += moviehtml.replace(/@img@/g, baseip + json.rows[i].imgpath).replace(/@id@/g, json.rows[i].id);
    }
...
function getMovieInfoBack(json) {
  ...
  $('#movieinfo_id').val(json.rows[0].id); //ID
  $('#movieinfo_img')
    .data('src', baseip + json.rows[0].imgpath)
    .attr('src', 'kernel/loadimg/wright.png')
    .addClass('load_img'); //封面图片
  $('#movieinfo_name').text(json.rows[0].title); //电影名字
  $('#movieinfo_info').text(json.rows[0].info); //电影简介
  $('#movieinfo_type').text(json.rows[0].type); //类型

  var star = parseInt(json.rows[0].star);
  var stars = '';
  for (var i = 0; i < star; i++) {
    stars += '★';
  }
  $('#movieinfo_star').text(stars); //星级
  $('#movieinfo_mytalk').text(json.rows[0].mytalk); //我的评语
  $('#movieinfo_downlink')
    .attr('href', json.rows[0].downlink)
    .text(json.rows[0].downinfo);

  var imghtml = '<img class="movieinfo_photo l_imgback load_img" data-src="@imgpath@" src="kernel/loadimg/wright.png" />';
  var str = '';
  for (var i = 0; i < json.rows[0].movieimgs.length; i++) {
    str += imghtml.replace(/@imgpath@/g, baseip + json.rows[0].movieimgs[i].imgpath);
  }
...
function getGameListBack(json) {
  ...
  for (var i = 0; i < json.rows.length; i++) {
    str += thehtml
      .replace(/@id@/g, json.rows[i].id)
      .replace(/@name@/g, json.rows[i].name)
      .replace(/@imgpath@/g, baseip + json.rows[i].imgpath);
  }
...
function gameOpenBack(json) {
  $('#yx_id').val(json.rows[0].id);
  $('#youxi_1')
    .css('background-image', 'url(' + baseip + json.rows[0].imgpath + ')')
    .text(json.rows[0].name);
  $('#yx_name').text(json.rows[0].name);
  $('#yx_type').text(json.rows[0].type);
  ...
  $('#yx_star').text(stars);
  $('#yx_howbig').text(json.rows[0].howbig);
  $('#yx_pz').text(json.rows[0].deploy);
  $('#yx_down')
    .attr('href', json.rows[0].downlink)
    .text(json.rows[0].downinfo);
  $('#yx_info').text(json.rows[0].info);
  $('#yx_mytalk').text(json.rows[0].mytalk);

  var imghtml = '<img src="kernel/loadimg/wright.png" data-src="@img@" class="y3_img load_img"/>';
  var str = '';
  for (var i = 0; i < json.rows[0].gameimg.length; i++) {
    str += imghtml.replace(/@img@/g, baseip + json.rows[0].gameimg[i].imgpath);
  }
...
function getArticleListBack(json) {
  ...
  for (var i = 0; i < json.rows.length; i++) {
    str += ahtml
      .replace(/@id@/g, json.rows[i].id)
      .replace(/@title@/g, json.rows[i].title)
      .replace(/@aur@/g, json.rows[i].author)
      .replace(/@imgpath@/g, baseip + json.rows[i].imgpath);
  }
...
function getOneArticleBack(json) {
  $('#wenzhangid').val(json.rows[0].ID);
  $('#w_title').text(json.rows[0].TITLE);
  $('#w_autor').text('————' + json.rows[0].AUTHOR);
  $('#w_body').html(json.rows[0].ARTICLEBODY);
  $('#wenzhang_photo').attr('src', baseip + json.rows[0].PHOTO);
  $('#w_autor2').text(json.rows[0].AUTHOR);
  $('#w_autorinfo').text(json.rows[0].INTR);
  $('#w_img').attr('src', baseip + json.rows[0].IMGPATH);
...
function getAllWorksBack(json) {
  ...
  for (var i = 0; i < json.rows.length; i++) {
    ...
    str2 += bigword
      .replace(/@num@/g, j)
      .replace(/@titleinfo@/g, json.rows[i].TITLEINFO)
      .replace(/@id@/g, json.rows[i].ID);
    str3 += bigbtn.replace(/@num@/g, j).replace(/@title@/g, json.rows[i].TITLE);
    j++;
  }
...
function openWorkBack(json) {
  ...
  var str = '';
  for (var i = 0; i < json.rows.length; i++) {
    if (json.rows[i].IMGPATH) {
      //有图片的显示图片
      str += img.replace(/@imgpath@/g, baseip + json.rows[i].IMGPATH);
    } else {
      //否则显示文字
      str += div.replace(/@info@/g, json.rows[i].INFO);
    }
  }
  $('#myworkbox').html(str);
  $('#mywork_t').val(json.rows[0].MYWORK_ID);
